...
wink.init({
    ...
    if ( auth_return === undefined ) {
        ...
        // success
        wink.user().device('Kitchen lights').power.off(function(response) {
...
### Added Methods
* **device_group:** *String* device type id: "light_bulbs"
* **device_id:** *Number* device identifier
* **power:**
    * **on(callback)**
    * **off(callback)**
    * **toggle(callback)** 
* **brightness(level, callback):** change brightness of lights
